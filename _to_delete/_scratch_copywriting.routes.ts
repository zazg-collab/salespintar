 ...
 * Sekarang mendukung 3 provider LLM (google/openai/openrouter) KHUSUS fitur Copywriting Ads --
 * pipeline audit video Video Guard TIDAK disentuh sama sekali, tetap Google-only spt sebelumnya
 * (sesuai instruksi eksplisit Bossfren: "cuman copywriting ads dlu"). Setting provider/key/model baru
 * disimpan di kolom TERPISAH (`copywritingLlmProvider`/`copywritingLlmApiKeyEncrypted`/
 * `copywritingLlmModel`) -- TAPI provider `google` tanpa key sendiri di kolom baru ini otomatis
 * fallback ke `metaGuardGeminiApiKeyEncrypted` yang sudah ada (lihat `resolveCopywritingLlmConfig`
 * di bawah), supaya business yang sudah mengisi Gemini key sebelum fitur ini ada TETAP jalan tanpa
 * perlu isi ulang apa pun -- backward compat penuh, bukan breaking change.
 *
...
// Panggilan LLM synchronous (dgn retry tenacity 3x exponential backoff di sisi metaguard_service,
// lihat _call_gemini_with_retry/_call_openai_with_retry) bisa makan waktu lebih lama drpd submit/poll
// job ringan video-guard (METAGUARD_TIMEOUT_MS 15s di sana) -- kasih ruang lebih longgar di sini krn
// request ini BLOCKING sampai LLM benar-benar selesai jawab (bukan async job spt /v1/audit).
const COPYWRITING_TIMEOUT_MS = 60_000;

type CopywritingLlmProvider = 'google' | 'openai' | 'openrouter';

const KNOWN_PROVIDERS: CopywritingLlmProvider[] = ['google', 'openai', 'openrouter'];
...
/** Resolve provider/API key/model Copywriting Ads per-business. Provider `google` TANPA key sendiri
 *  di `copywritingLlmApiKeyEncrypted` otomatis fallback ke `metaGuardGeminiApiKeyEncrypted` (kolom
 *  Video Guard yang sudah ada) -- lihat komentar docstring file di atas soal alasan backward compat
 *  ini. Provider `openai`/`openrouter` TIDAK punya fallback apa pun -- wajib diisi sendiri di setting
 *  Copywriting Ads (tidak ada key bersama server utk provider itu, beda dari Google yang punya
 *  fallback env generik di metaguard_service). */
async function resolveCopywritingLlmConfig(businessId: string): Promise<CopywritingLlmConfig> {
  ...
  const provider: CopywritingLlmProvider =
    rawProvider === 'openai' || rawProvider === 'openrouter' ? rawProvider : 'google';
