...
import { ArrowLeft, SlidersHorizontal, Loader2, Save, CheckCircle2, KeyRound, Info } from 'lucide-react';
...
const PROVIDERS: { key: Provider; label: string; defaultModel: string; keyHint: string }[] = [
  {
    key: 'google',
    label: 'Google AI Studio (Gemini)',
    defaultModel: 'gemini-2.5-flash',
    keyHint: 'Kosongkan untuk pakai key Video Guard yang sudah diisi (kalau ada) — satu key Gemini bisa dipakai bersama.',
  },
  {
    key: 'openai',
    label: 'OpenAI',
...
export default function CopywritingAdsSettingsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      body.model = modelInput.trim(); // "" = hapus/pakai default provider
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Pilih provider LLM untuk Check Ads &amp; Generate Ads. Pengaturan ini terpisah dari Gemini key
          Video Guard (audit video) — kecuali provider Google AI Studio dibiarkan tanpa key sendiri, yang
          otomatis memakai key Video Guard yang sama.
        </p>
      </div>
