...
class DirectorRemediationAdvice(BaseModel):
    b_roll_replacement: Optional[str] = None
    camera_angle: Optional[str] = None
    action_direction: Optional[str] = None
    environment_setting: Optional[str] = None
    wardrobe_props: Optional[str] = None
...
class RemediationAssets(BaseModel):
    ...
    # fix K5: v1.0.0 declared this a List[DirectorRemediationAdvice] but every worked example in the
    # same document showed a single object — one hook-window scene gets one director's note, not many.
    director_advice: Optional[DirectorRemediationAdvice] = None
