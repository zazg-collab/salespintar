...
export class LeadProfilerService {
   ...
   */
  private static compressForLlm(text: string): string {
    if (!text) return text;
  ...
  "draftWA": string
    ...
    try {
      ...
      if (isAfterSalesDomain) {
        ...
        // Langkah C Kelompok 2 (Dual-View, 2026-08-18, Temuan T1 -- FIXED): komentar lama di
        // sini (Fase 25) memperingatkan bahwa `rawTranscript` SUDAH head-tail-compressed oleh
        // pemanggil sebelum sampai kesini, jadi BUKAN riwayat penuh sungguhan. Itu sekarang
        // tidak berlaku lagi -- kedua pemanggil produksi (human-learning.service.ts jalur
        // realtime, reconciliation-sweeper.worker.ts jalur sweeper) sudah berhenti memotong di
        // hulu; `rawTranscript` di sini SEKARANG benar-benar riwayat penuh (maks 100 baris,
        // dibatasi LTRIM Fase 24) dari Redis. Kompresi head-tail cuma terjadi di
        // `compressForLlm()`, dipanggil khusus di titik payload LLM -- TIDAK PERNAH di jalur ini.
