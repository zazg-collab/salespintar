...
export class SessionBoundaryParser {
   ...
   */
  public static formatBufferLine(role: 'CS' | 'BUYER', text: string, timestampMs?: number): string {
    const cleanText = (text || '').replace(/\n/g, ' ');
    return timestampMs ? `[${role} ${timestampMs}] ${cleanText}` : `[${role}] ${cleanText}`;
  }
  ...
  public static isDeterministicClosing(transcript: string, buyerOnlyText?: string): boolean {
    ...
    // Jangan tandai closing jika PEMBELI mengirim sinyal purna jual (resi / barang sampai /
    // komplain / klaim garansi). CATATAN: "terima kasih banyak atas kepercayaannya" SENGAJA
    // dihapus dari sini — frasa itu adalah template CS DEAL_CONFIRMED (item 10), bukan sinyal
    // purna jual pembeli.
    //
    ...
    const isAfterSalesChat =
      /(resi|nomor\s+resi|status\s+pengiriman|kapan\s+sampai|belum\s+sampai|sudah\s+diterima|sdh\s+diterima|komplain|klaim\s+garansi|mau\s+retur|proses\s+retur|tukar\s+baru)/i.test(
        exclusionText,
      );
  ...
  public static parseLines(transcript: string): ParsedChatMessage[] {
    ...
    for (const rawLine of lines) {
      ...
      const tagMatch = rawLine.match(/^\[(CS|SELLER|ADMIN|BUYER|CUSTOMER|PEMBELI)(?:\s+(\d+))?\]:?\s*([\s\S]*)$/i);
      if (tagMatch) {
        const tag = tagMatch[1].toUpperCase();
        const tsRaw = tagMatch[2];
        const textContent = (tagMatch[3] || '').trim();
        const isCs = ['CS', 'SELLER', 'ADMIN'].includes(tag);
        let timestamp: Date | null = null;
        if (tsRaw) {
          const parsedTs = new Date(parseInt(tsRaw, 10));
