...
export default function DashboardSalesPage() {
  const [datePreset, setDatePreset] = useState<'today' | '7d' | '30d' | 'this_month' | 'custom'>('today');
  ...
  const summaryCards = [
    {
      ...
      value: `${totalAdLeads > 0 ? totalAdLeads : totalUniqueBuyers} Lead`,
    ...
    <div className="space-y-6 pb-12 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/app/leads"
            className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs md:text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-sm transition-colors"
          >
            <UserCheck className="w-4 h-4" />
            Riwayat Pelanggan
          </Link>
          <Link
            href="/app/learning-radar"
            className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs md:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors shadow-sm"
          >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex items-center gap-1.5 text-xs text-gray-600 font-semibold">
          ...
          <span className="text-gray-900 font-bold bg-gray-100 px-2 py-0.5 rounded">{dateRangeLabel}</span>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {summaryCards.map((card) => (
          <Link
            key={card.label}
            href={card.link}
            className={`bg-white p-3.5 rounded-xl border transition-all hover:shadow-md hover:border-indigo-200 group ${
          ...
          >
            ...
            <p className="text-[11px] font-semibold text-gray-700 mt-0.5">{card.label}</p>
