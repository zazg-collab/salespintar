 ...
 * yang salah.
 */
function lookupAlias(rawPlace) {
    var clean = normalizePlace(rawPlace);
    if (!clean)
        return null;
    if (exports.PLACE_ALIASES[clean])
        return exports.PLACE_ALIASES[clean];
    // Buang kata pembuka yang tidak menambah arti: "kota solo", "daerah jogja".
    var tanpaPembuka = clean.replace(/^(kota|kab|kabupaten|daerah|wilayah|kec|kecamatan)\s+/, '');
    if (tanpaPembuka !== clean && exports.PLACE_ALIASES[tanpaPembuka])
        return exports.PLACE_ALIASES[tanpaPembuka];
    return null;
}
