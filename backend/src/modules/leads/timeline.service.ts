...
export class TimelineService {
  ...
  static async getCustomerTimeline(businessId: string, rawPhone: string): Promise<CustomerTimelineResult | null> {
    ...
    for (let idx = 0; idx < leads.length; idx++) {
      ...
      // Langkah D-lanjutan (Fase 29, 2026-08-18, usulan Bossfren): nominal yg DIKETIK CS SENDIRI
      // saat konfirmasi COD ("TOTAL COD: Rp xxx") adalah acuan UTAMA -- jauh lebih akurat dari
      // tabel harga statis di bawah (yg cuma cover 17.6% SKU). Fallback ke katalog SKU, lalu ke
      // rata-rata Rp200rb, HANYA kalau CS tidak sempat menyebut angka di transkrip sama sekali.
      const estPrice = lead.confirmedCodAmount || ESTIMATED_PRICES[prodName] || (isClosing ? 200000 : 0);
