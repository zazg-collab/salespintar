...
[2026-08-26, Fase provider-agnostic] Sebelumnya modul ini HANYA lewat Google AI Studio (Gemini) pakai
API key mentah per-business. Sekarang mendukung 4 provider: `openai` (SDK resmi `openai`, Structured
Outputs `response_format=json_schema` strict), `openrouter` (SDK `openai` yang sama dgn base_url
override -- OpenRouter OpenAI-compatible, TAPI tidak semua model upstream di baliknya dukung
json_schema strict mode, jadi ada fallback ke `json_object` + instruksi schema di prompt kalau strict
ditolak, tetap divalidasi Pydantic sesudahnya spt biasa), `google` (genai.Client + response_schema
Pydantic asli, dgn API key per-business/fallback env -- TIDAK berubah dari versi sebelumnya), dan
**`agy` (DEFAULT sekarang, koreksi 2026-08-26)**.

Kenapa `agy` jadi default: ternyata Batch E (Check Ads + Generate Ads OTOMATIS tiap audit, lihat
main.py `_run_audit_job`) diam-diam memakai provider `google` (API key per-business) di setiap
audit, 1-2 panggilan per audit -- itu yang sebenarnya menghabiskan kuota free-tier Gemini API key
(20 request/hari) dgn cepat, BUKAN audit video-nya sendiri (yang sudah pakai `agy`/kuota Google AI
Pro subscription lewat AgyCliInvoker, tidak pernah menyentuh API key 20/hari itu). Koreksi Bossfren:
"token [API key] cuma dipakai di Check/Generate Ads" sebenarnya SUDAH benar sesuai desain awal, tapi
Batch E membuatnya jalan otomatis tiap audit -- jadi solusinya BUKAN nambah provider token alternatif
(openai/openrouter, tetap disediakan sbg opsi manual), tapi PINDAHKAN Check/Generate Ads ke `agy`
juga (SAMA PERSIS mekanisme video audit -- `api-bridge` `/v1/run`, kuota Google AI Pro subscription,
BUKAN API key per-request) supaya Batch E tidak pernah lagi menyentuh kuota 20/hari yang scarce itu.
`agy` TIDAK butuh API key dari business sama sekali. Dipisah pool `"copywriting-ads"` (BUKAN
`"video-guard"`) di api-bridge (pola sama dgn `ai-ads-fatigue`/`ai-ads-budget` yang sudah ada di
`ai-ads.routes.ts`) supaya tidak antre di belakang audit video yang bisa sampai 10 menit. Fungsi
`_call_agy_structured()` di bawah TIDAK menyentuh `engine.py`/`AgyCliInvoker` sama sekali -- cuma
meniru wire contract-nya (`POST /v1/run` + `JSON_DELIMITER` convention) dgn versi generik-schema,
krn `AgyCliInvoker` sendiri sebenarnya sudah schema-agnostic (yang hardcode `ComplianceRawAssessment`
adalah `parse_raw_assessment()`, fungsi terpisah, bukan kelas invoker-nya).

Scope SENGAJA cuma Copywriting Ads -- pipeline audit video/image (engine.py, GeminiSdkInvoker/
AgyCliInvoker, pool `"video-guard"`) TIDAK disentuh sama sekali.
...
LlmProvider = Literal["agy", "google", "openai", "openrouter"]

_KNOWN_PROVIDERS = ("agy", "google", "openai", "openrouter")

# Pool api-bridge KHUSUS Copywriting Ads (lihat AgyCliInvoker di engine.py yang pakai pool
# "video-guard") -- terpisah supaya Check/Generate Ads tidak antre FIFO di belakang audit video yang
# bisa makan waktu sampai 10 menit. Pola sama dgn "ai-ads-fatigue"/"ai-ads-budget" (ai-ads.routes.ts).
AGY_POOL = "copywriting-ads"

# Timeout dikirim ke api-bridge sbg timeoutMs (lihat AGY_CLI_TIMEOUT_MS di server.js, default global
# 300000ms) -- Copywriting Ads MURNI teks (tanpa video/gambar), jauh lebih cepat dari audit video,
# jadi budget waktu lebih pendek dari AgyCliInvoker punya video (600_000ms) sudah cukup longgar.
AGY_RUN_TIMEOUT_MS = 120_000

# Default model per provider -- dipakai kalau Business.settings.copywritingLlmModel kosong. `agy`
# TIDAK punya konsep model yang dipilih dari sini (agy/Antigravity CLI yang menentukan sendiri model
# di baliknya) -- key ini cuma placeholder biar dict lookup tidak KeyError, TIDAK PERNAH benar2
# dikirim ke mana pun utk provider agy. `google` TIDAK berubah dari sebelumnya (gemini-2.5-flash).
# `openai`/`openrouter` sengaja pilih model cepat+murah sbg default MVP, gampang diubah lewat setting
# per-business tanpa redeploy kalau ternyata kurang pas -- bukan keputusan final yang dikonfirmasi
# Bossfren, cuma starting point yang wajar.
DEFAULT_MODELS: dict = {
    "agy": "(otomatis -- ditentukan agy/Antigravity CLI sendiri, tidak bisa dipilih dari sini)",
...
def _is_transient_bridge_error(exc: BaseException) -> bool:
    # Sama persis heuristiknya dgn _is_transient_error (engine.py) -- murni cek substring pesan
    # exception, supaya konsisten. RuntimeError dari _call_agy_structured mengandung "HTTP {status}"
    # apa adanya (lihat raise di bawah), jadi "503"/"429" dari situ ikut ke-detect otomatis.
...
@retry(
    ...
    retry=retry_if_exception(_is_transient_bridge_error),
    reraise=True,
)
async def _call_agy_structured(system_instruction: str, prompt: str, schema_cls: type):
    """Provider `agy` -- meniru wire contract `AgyCliInvoker.generate()` (engine.py) TAPI generik
    atas `schema_cls` (AgyCliInvoker sendiri sebenarnya sudah schema-agnostic; yang hardcode
    ComplianceRawAssessment adalah `parse_raw_assessment()`, fungsi terpisah -- lihat docstring modul
    ini). TIDAK mengirim media_paths (Copywriting Ads murni teks, tidak pernah ada video/gambar) --
    jadi jauh lebih sederhana dari AgyCliInvoker.generate() yang harus symlink file ke AGY_WORKDIR.
    Retry cuma 2x attempt (bukan 3x spt Gemini) krn timeout agy sendiri sudah longgar (120s) dan ini
    fitur synchronous yang user tunggu langsung -- jangan bikin request menggantung terlalu lama."""
    settings = get_settings()
    if not settings.bridge_api_key:
        raise ValueError(
            "BRIDGE_API_KEY belum diset di metaguard_service -- provider 'agy' butuh akses ke "
            "api-bridge yang sama dengan Video Guard (lihat AgyCliInvoker di engine.py)."
        )
    ...
    timeout_s = (AGY_RUN_TIMEOUT_MS / 1000.0) + 30.0
    async with httpx.AsyncClient(timeout=timeout_s) as client:
        resp = await client.post(
            f"{settings.bridge_url.rstrip('/')}/v1/run",
            headers={"X-API-Key": settings.bridge_api_key},
            json={
                "prompt": full_prompt,
                "conversationId": f"copywriting-{uuid.uuid4()}",
                "timeoutMs": AGY_RUN_TIMEOUT_MS,
                "pool": AGY_POOL,
            },
        )
    if resp.status_code != 200:
        raise RuntimeError(f"api-bridge /v1/run gagal: HTTP {resp.status_code} -- {resp.text[:500]}")

    raw_text = resp.text
    text = raw_text.split(JSON_DELIMITER, 1)[1] if JSON_DELIMITER in raw_text else raw_text
    text = text.strip()
    # agy kadang membungkus JSON dalam ```json ... ``` walau sudah diminta tidak -- strip defensif,
    # SAMA PERSIS pola parse_raw_assessment() di engine.py.
    text = re.sub(r"^```(?:json)?\s*|\s*```$", "", text.strip())
    try:
        return schema_cls.model_validate_json(text)
    except Exception as e:
        # Beda dari parse_raw_assessment() (yang fallback ke MANUAL_REVIEW krn ComplianceRawAssessment
        # punya bentuk "kosong tapi valid" utk kasus itu) -- CopywritingCheckResult/GenerateResult
        # TIDAK punya padanan itu, jadi kegagalan parse dilempar sbg ValueError yang jelas (ditangkap
        # jadi HTTP 400 di main.py) drpd menyamarkan sbg hasil AMAN/kosong yang menyesatkan.
        raise ValueError(
            f"Gagal parse respons agy sbg {schema_cls.__name__} ({e}). Cuplikan mentah: {raw_text[:300]!r}"
        ) from e
...
async def _generate_structured(
    ...
    tahu-menahu provider mana yang dipakai di baliknya. Fungsi ini `async` (walau google/openai/
    openrouter di bawah semua panggilan SDK sync) khusus supaya provider `agy` bisa `await` httpx
    AsyncClient-nya sendiri -- caller (run_copywriting_check/generate) WAJIB `await` hasil fungsi ini."""
    if provider == "agy":
        return await _call_agy_structured(system_instruction, prompt, schema_cls)
